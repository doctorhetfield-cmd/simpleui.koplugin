...
--   1. Copy locale/folio.pot → locale/<lang>.po  (e.g. locale/de.po)
...
-- ---------------------------------------------------------------------------
-- .po parser — minimal, handles the subset generated by folio.pot
-- ---------------------------------------------------------------------------
...
local function loadTranslations()
    ...
    local function try(name)
        ...
        if t and next(t) then
            ...
            logger.info("folio i18n: loaded " .. path .. " — " .. n .. " strings")
            return t
        end
    end
...
local function install()
    ...
    if not orig_gettext then
        ...
        if not ok or not gt then
            logger.warn("folio i18n: cannot load gettext, translations disabled")
            return
        end
        orig_gettext = gt
    end
    ...
    elseif type(orig_gettext) == "function" then
        ...
        logger.warn("folio i18n: unexpected gettext type: " .. type(orig_gettext))
        return
    end

    package.loaded["gettext"] = wrapper
    _installed = true
    logger.info("folio i18n: installed wrapper for language: " .. detectLang())
end
...
local function uninstall()
    ...
    _installed    = false
    _translations = nil
    logger.info("folio i18n: uninstalled")
end
