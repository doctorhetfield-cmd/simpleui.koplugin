...
local _InfoMessage
local function InfoMessage()
	_InfoMessage = _InfoMessage or require("ui/widget/infomessage")
	return _InfoMessage
end
...
function M.BAR_H()
	return _cached("bar_h", function()
		return math.floor(Screen:scaleBySize(96) * _getNavbarScale())
	end)
end
function M.ICON_SZ()
	return _cached("icon_sz", function()
		return math.floor(Screen:scaleBySize(44) * _getNavbarScale() * (Config.getIconScalePct() / 100))
	end)
end
function M.ICON_TOP_SP()
	return _cached("it_sp", function()
		return math.floor(Screen:scaleBySize(10) * _getNavbarScale())
	end)
end
function M.ICON_TXT_SP()
	return _cached("itxt_sp", function()
		return math.floor(Screen:scaleBySize(4) * _getNavbarScale())
	end)
end
function M.LABEL_FS()
	return _cached("lbl_fs", function()
		return math.floor(Screen:scaleBySize(9) * _getNavbarScale() * (Config.getLabelScalePct() / 100))
	end)
end
function M.INDIC_H()
	return _cached("indic_h", function()
		return math.floor(Screen:scaleBySize(3) * _getNavbarScale())
	end)
end

-- Structural dimensions — not affected by the size setting.
function M.TOP_SP()
	return _cached("top_sp", function()
		return Screen:scaleBySize(2)
	end)
end
function M.BOT_SP()
	return _cached("bot_sp", function()
		return math.floor(Screen:scaleBySize(12) * Config.getBottomMarginPct() / 100)
	end)
end
function M.SIDE_M()
	return _cached("side_m", function()
		return Screen:scaleBySize(24)
	end)
end
function M.SEP_H()
...
function M.getPaginationIconSize()
	...
	if key == "xs" then
		return Screen:scaleBySize(20)
	elseif key == "s" then
		return Screen:scaleBySize(28)
	else
...
function M.getTabWidths(num_tabs, usable_w)
	...
	for i = num_tabs + 1, #_tab_widths_cache do
		_tab_widths_cache[i] = nil
	end
...
function M.buildTabCell(action_id, active, tab_w, mode)
	...
	if not _vspan_icon_top then
		_vspan_icon_top = VerticalSpan:new({ width = M.ICON_TOP_SP() })
	end
	...
	if mode == "text" or mode == "both" then
		if mode == "both" then
			if not _vspan_icon_txt then
				_vspan_icon_txt = VerticalSpan:new({ width = M.ICON_TXT_SP() })
			end
...
function M.updateNavpagerArrows(widget, has_prev, has_next)
	...
	if not bar._navpager_has_arrows then
		return false
	end
	...
	if not (prev_cc and prev_cc._arrow_enabled ~= nil and next_cc and next_cc._arrow_enabled ~= nil) then
		return false
	end
	...
	local function _apply(cc, enabled)
		if cc._arrow_enabled == enabled then
			return
		end
		cc._arrow_enabled = enabled
...
function M.buildBarWidget(active_action_id, tab_config, num_tabs, mode)
	...
	if Config.isNavpagerEnabled() then
		...
		return M.buildBarWidgetWithArrows(active_action_id, tab_config, mode, has_prev, has_next)
	end
...
function M.registerTouchZones(plugin, fm_self)
	...
	logger.dbg(
		"simpleui tz: registerTouchZones on=",
		tostring(fm_self and fm_self.name),
		"navpager=",
		tostring(navpager),
		"num_tabs=",
		tostring(num_tabs)
	...
	local function _callPageFn(fn_name)
		...
		if fm_inst and fm_inst.file_chooser then
			pcall(function()
				fm_inst.file_chooser[fn_name](fm_inst.file_chooser)
			end)
		end
	end
	...
	zones[#zones + 1] = {
		...
		handler = function(ges)
			...
			if not plugin._makeNavbarMenu then
				plugin:addToMainMenu({})
			end
			local UI_mod = require("sui_core")
			local topbar_on = G_reader_settings:nilOrTrue("navbar_topbar_enabled")
			local top_offset = topbar_on and require("sui_topbar").TOTAL_TOP_H() or 0
			UI_mod.showSettingsMenu(_("Bottom Bar"), plugin._makeNavbarMenu, top_offset, screen_h, M.TOTAL_H())
			return true
		end,
	}
...
function M.onTabTap(plugin, action_id, fm_self)
	...
	if _ACTION_ONLY[action_id] then
		...
		elseif action_id == "bookmark_browser" then
			M.showBookmarkBrowserSourceDialog(plugin.ui)
		end
		return
	end
	...
	if fm_self._navbar_container and action_id ~= "homescreen" and not hs_open and not already_active then
...
local function _isInPlaceAction(action_id)
	...
	if action_id == "bookmark_browser" then
		return true
	end
			...
			{
				{
					...
					callback = function()
						open_with_source(home_dir, false)
					end,
				},
			},
			{
				{
					text = _("Home folder + subfolders"),
					enabled = home_dir ~= nil,
					callback = function()
						open_with_source(home_dir, true)
					end,
				},
			},
			{
...
local function _executeInPlace(action_id, plugin, fm)
	...
	elseif action_id == "stats_calendar" then
		...
		if not ok then
			showUnavailable(_("Statistics plugin not available."))
		end
	elseif action_id == "bookmark_browser" then
...
function M.navigate(plugin, action_id, fm_self, tabs, force)
	...
	if fm_self ~= fm then
		...
		pcall(function()
			if fm_self.onCloseAllMenus then
				fm_self:onCloseAllMenus()
			elseif fm_self.onClose then
				fm_self:onClose()
			end
		end)
	...
	elseif action_id == "collections" then
		if fm.collections then
			...
			showUnavailable(_("Collections not available."))
		end
	elseif action_id == "history" then
		...
		if not ok then
			showUnavailable(_("History not available."))
		end
	...
	elseif action_id == "favorites" then
		if fm.collections then
			...
			showUnavailable(_("Favorites not available."))
		end
	elseif action_id == "bookmark_browser" then
...
function M.doWifiToggle(plugin)
	local ok_hw, has_wifi = pcall(function()
...
function M.showFrontlightDialog()
	local ok_f, has_fl = pcall(function()
		return Device:hasFrontlight()
	end)
	if not ok_f or not has_fl then
		UIManager:show(InfoMessage():new({
			text = _("Frontlight not available on this device."),
			timeout = 2,
		}))
		return
	end
...
function M.rebuildAllNavbars(plugin)
	if plugin and plugin._simpleui_suspended then
		return
	end
	...
	local function rebuildWidget(w)
		if not w or not w._navbar_container or seen[w] then
			return
		end
		seen[w] = true
	...
	local ok_icon, err_icon = pcall(function()
		plugin:_updateFMHomeIcon()
	end)
	if not ok_icon then
		logger.warn("simpleui: _updateFMHomeIcon failed:", tostring(err_icon))
	end
	for _i, entry in ipairs(stack) do
		local ok, err = pcall(rebuildWidget, entry.widget)
		if not ok then
			logger.warn("simpleui: rebuildWidget failed:", tostring(err))
		end
	end
end
...
function M.setPowerTabActive(plugin, active, prev_action)
	...
	for _i, entry in ipairs(stack) do
		...
		if not ok then
			logger.warn("simpleui: setPowerTabActive updateWidget failed:", tostring(err))
		end
	end
end
...
function M.rewrapAllWidgets(plugin)
	...
	local function rewrapWidget(w)
		...
		if w._navbar_injected then
			...
			if w[1] and w[1].dimen then
				w[1].dimen.h = content_h
				w[1].dimen.y = content_y
			end
	...
	for _i, entry in ipairs(stack) do
		...
		if not ok then
			logger.warn("simpleui: rewrapWidget failed:", tostring(err))
		end
	end
end
...
function M.restoreTabInFM(plugin, tabs, prev_action)
	...
	local restored = (fm.file_chooser and Patches._resolveTabForPath(fm.file_chooser.path, t)) or prev_action or t[1]
...
function M.showPowerDialog(plugin)
	if plugin._power_dialog then
		return
	end -- guard: ignore double-tap
	local ButtonDialog = require("ui/widget/buttondialog")
	...
	-- Create a table of available power options
	local power_buttons_table = {}
	if Device:canRestart() then
		table.insert(power_buttons_table, {
			{
				text = _("Restart"),
				callback = function()
					_quitting = true
	...
	-- Display the options to Restart, Sleep, or Quit, based on what the Device supports
