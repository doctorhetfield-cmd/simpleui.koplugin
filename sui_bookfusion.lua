...
-- Mirrors bf_browser's "is this book already on disk?" probe by calling the
-- same resolver it does (bf_downloader.resolveDownloadedPath), rather than
-- rebuilding the path ourselves.
--
-- We used to probe getDownloadDir() .. "/" .. buildFilename(book) directly.
-- In v1.1.2 that is only the *fallback* branch of the resolver: the per-book
-- path recorded at download time is checked first. Two ways the old probe
-- disagreed with the plugin - both showing a cloud badge on a book that is
-- actually on the device, so the badge contradicted the dialog that opened
-- when you tapped it:
--   * "Choose folder" downloads set the last-used dir, so a book downloaded
--     to a different folder than the current default was never found.
--   * buildFilename now goes through util.getSafeFilename and preserves
--     non-ASCII titles, so names generated by the old plugin no longer match
--     the ones we computed.
--
-- Note this is not a pure read: the resolver clears stale entries and
-- back-registers legacy downloads it discovers. That self-healing is the
-- point, but it is why callers hoist the badge-enabled check rather than
-- calling this per tile unconditionally.
function Data.isDownloaded(book)
    ...
    if not ok_dl or not Downloader or not Downloader.resolveDownloadedPath then
        return false
    end
    local p = Data.getPlugin()
    local settings = p and p.bf_settings or nil
    local ok, path = pcall(Downloader.resolveDownloadedPath, settings, book)
...
function Data.selectBook(book, on_change)
    ...
    local ok, err = pcall(function()
        ...
        -- bf_browser v1.1.2 calls findBookIndex() in two places we reach:
        -- onSelectBook (to remember the row to refresh after a download) and
        -- the "Remove from device" confirm callback. It iterates
        -- self._bl_books, which only openBookList populates - and this is a
        -- headless throwaway instance that never opens a list, so the field is
        -- nil and ipairs() raises.
        --
        -- The download path raised inside our pcall, so tapping any cloud-only
        -- book silently did nothing. The remove path was worse: there
        -- findBookIndex is evaluated as an *argument*, so stubbing
        -- refreshBookList below cannot intercept it, and it runs from a
        -- ConfirmBox callback long after this pcall has returned - deleting the
        -- file and clearing the settings entry, then throwing uncaught.
        --
        -- An empty table makes findBookIndex return nil, which both call sites
        -- already handle.
        browser._bl_books = {}
...
--   L2: HTTP fetch via bf_image_loader (async, forked subprocess) → on
--       success, write to disk cache and fire callback to repaint.
--
-- As of bf plugin v1.1.2 the L2 transport is bf_http.batch: one forked child
-- holding a single keep-alive connection for the whole batch, streaming
-- results back over a pipe. It is not dismissable by UI input, so the old
-- caveat about covers needing several refreshes to land while the user is
-- interacting no longer applies.
-- ===========================================================================
...
--
-- No retry pass: bf_image_loader v1.1.2 streams covers through bf_http.batch
-- (one forked child holding one keep-alive connection, results piped back)
-- instead of Trapper. Nothing is dismissable by UI events any more, and
-- bf_http already retries per connection, so there is nothing to recover from.
--
-- The retry machinery that used to live here had also stopped working:
-- loadImages now returns (nil, halt) - the batch handle is gone - so the
-- poll's `if not current_batch` guard was always true. It rescheduled itself
-- every second for the lifetime of the tab and never reached the retry logic
-- below it.
function Covers.fetchMissing(urls, on_done, opts)
    ...
    start_fn = function()
        ...
        local _batch, halt = ImageLoader:loadImages(missing, on_fetch)
        inner_halt = halt
    end
    if defer then
...
-- user had open when they tapped ↻.  Order still matters — bf_http.batch
-- walks the request list in order on its single connection — so listing
-- Currently Reading first fills the visible carousel before anything else.
-- (Pre-v1.1.2 this was a Trapper queue with a 0.2 s inter-request gap;
-- the ordering rationale survives, the pacing does not.)
